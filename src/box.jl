...
Rules for reverse-mode differentiation (`rrules`) below just let the AD system auto-differentiate the Box
contstructor and `surfpt_nearby` method for `Box` subtypes, but then re-format the gradient result as a `Tangent{Box}`,
