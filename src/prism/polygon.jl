...
"""
ChainRulesCore differentiation rules for Polygon

Rules for reverse-mode differentiation (`rrules`) below just let the AD system auto-differentiate the Polygon
contstructor and `surfpt_nearby` method for Polygons, but then re-format the gradient result as a `Tangent{Polygon}`,
